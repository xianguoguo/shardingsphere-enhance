...
import io.shardingsphere.core.routing.type.complex.YComplexRoutingEngine;
...
import io.shardingsphere.core.routing.type.standard.YStandardRoutingEngine;
 ...
 * 路由引擎工厂。替换sharding-core包中的相同包下的相同类实现。
 * @author liuqiangm
 */
public final class RoutingEngineFactory {
  ...
  public static RoutingEngine newInstance(final ShardingRule shardingRule,
      final ShardingDataSourceMetaData shardingDataSourceMetaData, final SQLStatement sqlStatement,
      final ShardingConditions shardingConditions) {
    Collection<String> tableNames = sqlStatement.getTables().getTableNames();
    RoutingEngine result;
    if (sqlStatement instanceof UseStatement) {
      result = new IgnoreRoutingEngine();
    } else if (shardingRule.isAllBroadcastTables(tableNames)
        && !(sqlStatement instanceof SelectStatement)) {
      result = new DatabaseBroadcastRoutingEngine(shardingRule);
    } else if (sqlStatement instanceof DDLStatement || (sqlStatement instanceof DCLStatement
        && ((DCLStatement) sqlStatement).isGrantForSingleTable())) {
      result = new TableBroadcastRoutingEngine(shardingRule, sqlStatement);
    } else if (sqlStatement instanceof ShowDatabasesStatement || (
        (sqlStatement instanceof ShowTablesStatement
            || sqlStatement instanceof ShowTableStatusStatement) && tableNames.isEmpty())
      ...
      result = new YStandardRoutingEngine(shardingRule, tableNames.iterator().next(),
          shardingConditions, sqlStatement instanceof DQLStatement);
    } else {
      // TODO config for cartesian set
      result = new YComplexRoutingEngine(shardingRule, tableNames, shardingConditions,
          sqlStatement instanceof DQLStatement);
    }
    return result;
  }
}
