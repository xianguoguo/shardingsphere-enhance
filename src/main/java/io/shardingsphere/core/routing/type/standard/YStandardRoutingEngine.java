 ...
 * @author panjuan
 */
public final class YStandardRoutingEngine implements RoutingEngine {
  ...
  public YStandardRoutingEngine(ShardingRule shardingRule, String logicTableName,
  ...
  public RoutingResult route() {
    return generateRoutingResult(
        getDataNodes(shardingRule.getTableRuleByLogicTableName(logicTableName)));
  }
  ...
  private Collection<DataNode> routeByShardingConditions(final TableRule tableRule) {
    return shardingConditions.getShardingConditions().isEmpty() ? route(tableRule,
        Collections.<ShardingValue>emptyList(), Collections.<ShardingValue>emptyList())
  ...
  private Collection<DataNode> routeByShardingConditionsWithCondition(final TableRule tableRule) {
    ...
    for (ShardingCondition each : shardingConditions.getShardingConditions()) {
      Collection<DataNode> dataNodes = route(tableRule, getShardingValuesFromShardingConditions(
  ...
  private Collection<DataNode> routeByMixedConditions(final TableRule tableRule) {
    return shardingConditions.getShardingConditions().isEmpty() ? routeByMixedConditionsWithHint(
        tableRule) : routeByMixedConditionsWithCondition(tableRule);
  }

  private Collection<DataNode> routeByMixedConditionsWithCondition(final TableRule tableRule) {
    Collection<DataNode> result = new LinkedList<>();
    for (ShardingCondition each : shardingConditions.getShardingConditions()) {
      Collection<DataNode> dataNodes = route(tableRule, getDatabaseShardingValues(tableRule, each),
          getTableShardingValues(tableRule, each));
      reviseShardingConditions(each, dataNodes);
      result.addAll(dataNodes);
    }
    return result;
  ...
  private Collection<DataNode> routeByMixedConditionsWithHint(final TableRule tableRule) {
    if (shardingRule.getDatabaseShardingStrategy(tableRule) instanceof HintShardingStrategy) {
      return route(tableRule, getDatabaseShardingValuesFromHint(),
          Collections.<ShardingValue>emptyList());
    }
    return route(tableRule, Collections.<ShardingValue>emptyList(),
        getTableShardingValuesFromHint());
  }

  private List<ShardingValue> getDatabaseShardingValues(final TableRule tableRule,
      final ShardingCondition shardingCondition) {
    ...
    return isGettingShardingValuesFromHint(dataBaseShardingStrategy)
        ...
        : getShardingValuesFromShardingConditions(dataBaseShardingStrategy.getShardingColumns(),
            shardingCondition);
  }
  ...
  private List<ShardingValue> getTableShardingValues(final TableRule tableRule,
    ...
    return isGettingShardingValuesFromHint(tableShardingStrategy)
        ? getTableShardingValuesFromHint()
        : getShardingValuesFromShardingConditions(tableShardingStrategy.getShardingColumns(),
            shardingCondition);
  }
  ...
  private List<ShardingValue> getDatabaseShardingValuesFromHint() {
    Optional<ShardingValue> shardingValueOptional = HintManagerHolder
        .getDatabaseShardingValue(logicTableName);
    return shardingValueOptional.isPresent() ? Collections
        .singletonList(shardingValueOptional.get()) : Collections.<ShardingValue>emptyList();
  }

  private List<ShardingValue> getTableShardingValuesFromHint() {
    Optional<ShardingValue> shardingValueOptional = HintManagerHolder
        .getTableShardingValue(logicTableName);
    return shardingValueOptional.isPresent() ? Collections
        .singletonList(shardingValueOptional.get()) : Collections.<ShardingValue>emptyList();
  }

  private List<ShardingValue> getShardingValuesFromShardingConditions(
      final Collection<String> shardingColumns, final ShardingCondition shardingCondition) {
    List<ShardingValue> result = new ArrayList<>(shardingColumns.size());
    for (ShardingValue each : shardingCondition.getShardingValues()) {
      Optional<BindingTableRule> bindingTableRule = shardingRule
          .findBindingTableRule(logicTableName);
      if ((logicTableName.equals(each.getLogicTableName())
          || bindingTableRule.isPresent() && bindingTableRule.get().hasLogicTable(logicTableName))
  ...
  /**
   * 根据分片规则进行分库分表操作的实际路由节点。 此处可以用于判断是否发生了分片
   *
   * @param tableShardingValues logicSQL实际执行的数据节点。
   */
  private Collection<DataNode> route(final TableRule tableRule,
      final List<ShardingValue> databaseShardingValues,
      final List<ShardingValue> tableShardingValues) {
    String defaultDataSourceName = shardingRule.getShardingRuleConfig().getDefaultDataSourceName();
    ...
    // 判断是否实际发生了分库或者分表
    boolean shardingFlag = false;
    for (DataNode node : result) {
      if (!logicTableName.equals(node.getTableName()) || !defaultDataSourceName
          .equals(node.getDataSourceName())) {
        shardingFlag = true;
      }
    }
    if (shardingFlag && dqlStatement && ShardingConfiguration.isReadOld(logicTableName)) {
      // 读取源控制
      result.clear();
      result.add(new DataNode(defaultDataSourceName, logicTableName));
    } else if (shardingFlag && !dqlStatement && ShardingConfiguration
        .isDoubleModify(logicTableName)) {
      // 双写控制
      result.add(new DataNode(defaultDataSourceName, logicTableName));
    }
    return result;
  ...
  private Collection<String> routeDataSources(final TableRule tableRule,
    ...
    Collection<String> result = new LinkedHashSet<>(
        shardingRule.getDatabaseShardingStrategy(tableRule)
            .doSharding(availableTargetDatabases, databaseShardingValues));
  ...
  private void reviseShardingConditions(final ShardingCondition each,
